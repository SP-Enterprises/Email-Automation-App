...
app = Flask(__name__)
required_env_vars = ['SECRET_KEY', 'MAIL_USERNAME', 'MAIL_PASSWORD', 'FERNET_KEY']
    ...
    with open(log_file_path, mode="w", newline="", encoding="utf-8") as log_file:
        ...
        for _, row in df.iterrows():
            ...
            try:
                ...
                send_email_with_retry(email, subject, rendered_html)
                ...
                return jsonify({"message": f"Failed to send email to {email}: {str(e)}"}), 500         
            ...
            email=email,
            status="Success" if not e else "Failed",
...
@login_required
def download_log():
    logs = EmailLog.query.filter_by(user_id=current_user.id).all()
    output = io.StringIO()
    writer = csv.writer(output)
    writer.writerow(["Company Name", "Email", "Status", "Timestamp", "Error"])
    for log in logs:
        writer.writerow([log.company_name, log.email, log.status, log.timestamp, log.error])
    output.seek(0)
    return send_file(
